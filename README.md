...
- Creates VMs for `masters`, `workers`, and optional `storage_workers` using `bpg/proxmox`.
- Downloads Talos `nocloud` raw images (`.raw.zst`) on every Proxmox node that hosts at least one VM.
...
module "talos" {
  ...
  storage_workers = [
    # iSCSI-capable datastore: scsi0 is system disk, scsi1 attaches the LUN
    {
      host            = "st-01"
      proxmox_node    = "pve2"
      ip_mode         = "dhcp"
      ram_mb          = 16384
      cpu_cores       = 8
      datastore_id    = "iscsi-prod"
      system_datastore_id = "local-lvm"
      efi_datastore_id = "local-lvm"
      cloud_init_datastore_id = "local-lvm"
      iscsi_disk_name = "lun-01"
      iscsi_disk_gb   = 500
    },

    # non-iSCSI datastore: only scsi0 system disk is created
    {
      host         = "st-02"
      proxmox_node = "pve3"
      ip_mode      = "dhcp"
...
- If a `storage_workers[*]` node uses a datastore listed in `proxmox_iscsi_datastores`, `iscsi_disk_name` is required and must point to the target iSCSI LUN.
- If a `storage_workers[*]` node uses iSCSI datastore, `iscsi_disk_gb` is required and should match the LUN size in Proxmox.
- If a `storage_workers[*]` node uses iSCSI datastore, `system_datastore_id` is required for the system disk (`scsi0`).
- `storage_workers[*].system_datastore_id` controls where the system disk (`scsi0`) is created.
- `storage_workers[*].efi_datastore_id` and `storage_workers[*].cloud_init_datastore_id` can override where EFI and cloud-init disks are created.
...
- Storage workers receive their storage metadata from Talos machine config: label `node.kubernetes.io/assignment=storage` via `machine.nodeLabels` and initial taint `node.kubernetes.io/assignment=storage:PreferNoSchedule` via `machine.kubelet.extraConfig.registerWithTaints`.
- The module intentionally uses `node.kubernetes.io/assignment=storage` instead of `node-role.kubernetes.io/storage` because Kubernetes `NodeRestriction` can reject node-applied role labels.
...
| [talos_machine_configuration_apply.storage_worker](https://registry.terraform.io/providers/siderolabs/talos/latest/docs/resources/machine_configuration_apply) | resource |
...
| [talos_cluster_health.control_plane_ready](https://registry.terraform.io/providers/siderolabs/talos/latest/docs/data-sources/cluster_health) | data source |
...
| <a name="input_proxmox_efi_pre_enrolled_keys"></a> [proxmox\_efi\_pre\_enrolled\_keys](#input\_proxmox\_efi\_pre\_enrolled\_keys) | Whether to enable pre-enrolled secure boot keys on EFI disk (recommended for Talos secure boot images) | `bool` | `true` | no |
| <a name="input_proxmox_iscsi_datastores"></a> [proxmox\_iscsi\_datastores](#input\_proxmox\_iscsi\_datastores) | Datastores that should be treated as iSCSI-compatible for storage\_workers | `set(string)` | `[]` | no |
| <a name="input_proxmox_iso_datastore"></a> [proxmox\_iso\_datastore](#input\_proxmox\_iso\_datastore) | Proxmox datastore where the Talos disk image is downloaded | `string` | `"local"` | no |
...
| <a name="input_storage_worker_default_system_disk_gb"></a> [storage\_worker\_default\_system\_disk\_gb](#input\_storage\_worker\_default\_system\_disk\_gb) | Default system disk size (GB) for storage workers when a node does not define disk\_gb | `number` | `80` | no |
| <a name="input_storage_workers"></a> [storage\_workers](#input\_storage\_workers) | Storage worker nodes definition | <pre>list(object({<br/>    host         = string<br/>    proxmox_node = string<br/><br/>    ip_mode = optional(string, "dhcp")<br/>    ip      = optional(string)<br/>    cidr    = optional(number)<br/>    gateway = optional(string)<br/><br/>    ram_mb    = number<br/>    cpu_cores = number<br/>    cpu_type  = optional(string)<br/><br/>    disk_gb                 = optional(number)<br/>    datastore_id            = optional(string)<br/>    system_datastore_id     = optional(string)<br/>    efi_datastore_id        = optional(string)<br/>    cloud_init_datastore_id = optional(string)<br/>    iscsi_disk_name         = optional(string)<br/>    iscsi_disk_gb           = optional(number)<br/>    pool_id                 = optional(string)<br/>    bridge                  = optional(string)<br/>    vlan_id                 = optional(number)<br/>    mac_address             = optional(string)<br/><br/>    install_disk           = optional(string)<br/>    machine_config_patches = optional(list(string), [])<br/>  }))</pre> | `[]` | no |
...
| <a name="input_wait_for_control_plane_health"></a> [wait\_for\_control\_plane\_health](#input\_wait\_for\_control\_plane\_health) | Whether to run final Talos cluster health checks after all machine configurations are applied | `bool` | `true` | no |
